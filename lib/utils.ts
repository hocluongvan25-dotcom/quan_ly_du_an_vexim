...
// FDA: ngày hết hạn tính từ số năm đã chọn; GACC: cố định 5 năm
export function expiryFromStandard(
  registeredAt: string,
  standard: Standard,
  validityYears?: number | null
) {
  if (standard === "GACC") return addYears(registeredAt, GACC_FIXED_YEARS);
  const years =
...
export function getValidityYears(cert: {
  ...
  if (cert.standard === "GACC") return GACC_FIXED_YEARS;
